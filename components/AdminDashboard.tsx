...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onLogout }) => {
  const [activeTab, setActiveTab] = useState<'stats' | 'monitoring' | 'subjects' | 'users'>('stats');
  ...
  const exportToCsv = () => {
    ...
    alert('File CSV berhasil diunduh. Gunakan menu "File > Impor" di Google Sheets untuk membuka file ini.');
  };
                    ...
                    <tr key={exam.id} className="border-b hover:bg-slate-50 transition group">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition">
                          <button 
                            onClick={() => forceFinishExam(exam, 'completed')}
                          ...
                          <button 
                            onClick={() => forceFinishExam(exam, 'terminated')}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className="bg-white text-emerald-600 border border-emerald-600 px-4 py-2 rounded-lg font-bold flex items-center gap-2 hover:bg-emerald-50 transition"
                    >
                  ...
                  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-md p-4">
                    <div className="bg-white rounded-3xl p-8 max-w-2xl w-full shadow-2xl animate-in zoom-in-95">
                       <div className="flex justify-between items-center mb-6">
                          <h3 className="text-2xl font-black text-emerald-700">Hubungkan ke Google Sheets</h3>
                          <button onClick={() => setIsGuideOpen(false)} className="text-slate-400 hover:text-red-500 text-2xl font-bold">&times;</button>
                       </div>
                       <div className="space-y-6 text-slate-700">
                          <div className="flex gap-4">
                             <div className="w-10 h-10 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center flex-shrink-0 font-bold">1</div>
                             <div>
                                <p className="font-bold mb-1">Ekspor Data CSV</p>
                                <p className="text-sm">Klik tombol <span className="text-emerald-600 font-bold uppercase">"Sync to Google Sheets"</span> di dashboard ini untuk mengunduh file hasil ujian.</p>
                             </div>
                          </div>
                          <div className="flex gap-4">
                             <div className="w-10 h-10 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center flex-shrink-0 font-bold">2</div>
                             <div>
                                <p className="font-bold mb-1">Buka Google Sheets</p>
                                <p className="text-sm">Buat spreadsheet baru atau buka yang sudah ada di <span className="text-indigo-600 font-mono">sheets.google.com</span>.</p>
                             </div>
                          </div>
                          <div className="flex gap-4">
                             <div className="w-10 h-10 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center flex-shrink-0 font-bold">3</div>
                             <div>
                                <p className="font-bold mb-1">Impor File</p>
                                <p className="text-sm">Buka menu <span className="font-bold">File &gt; Import (Impor)</span>, pilih tab <span className="font-bold">Upload (Upload)</span>, dan pilih file CSV yang baru saja diunduh.</p>
                             </div>
                          </div>
                          <div className="flex gap-4">
                             <div className="w-10 h-10 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center flex-shrink-0 font-bold">4</div>
                             <div>
                                <p className="font-bold mb-1">Atur Pemisah (Separator)</p>
                                <p className="text-sm">Pilih <span className="font-bold">"Comma" (Koma)</span> sebagai pemisah agar data terbagi ke kolom yang tepat secara otomatis.</p>
                             </div>
                          </div>
                          <div className="p-4 bg-amber-50 rounded-xl border border-amber-200 text-xs text-amber-800 leading-relaxed italic">
                             <strong>Tips Pro:</strong> Anda dapat menggunakan rumus <code>=IMPORTDATA("link_csv_anda")</code> jika file tersebut disimpan di server publik untuk update data secara otomatis tanpa impor manual.
                          </div>
                       </div>
                       <div className="mt-8">
                          <button onClick={() => setIsGuideOpen(false)} className="w-full bg-emerald-600 text-white py-4 rounded-xl font-black shadow-lg hover:bg-emerald-700 transition">SAYA MENGERTI</button>
                       </div>
                    </div>
                  </div>
                )}
